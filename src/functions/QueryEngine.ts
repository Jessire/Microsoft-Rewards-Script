...
export class QueryCore {
    ...
    async queryManager(options: QueryManagerOptions = {}): Promise<string[]> {
        ...
        try {
            ...
            const sourceHandlers: Record<QueryEngine, () => Promise<string[]> | string[]> = {
                ...
                china: () => this.getChinaTrends().catch(() => [])
            }
    ...
    async getChinaTrends(): Promise<string[]> {
        const appkey = this.bot.config.searchSettings.chinaApi?.appkey?.trim()
        const sources = [...CHINA_HOT_SOURCES]
        this.bot.utils.shuffleArray(sources)
        const preferredCount = appkey ? 2 : 1
        const orderedSources = [...sources.slice(0, preferredCount), ...sources.slice(preferredCount)]
        let rateLimited = false

        for (const source of orderedSources) {
            if (rateLimited && !appkey) await new Promise(resolve => setTimeout(resolve, randomChinaDelay()))

            try {
                const response = await this.bot.http.request<unknown>(
                    {
                        url: buildChinaApiUrl(source, appkey),
                        method: 'GET',
                        headers: {
                ...
                if (isChinaRateLimited(response.status, response.data)) {
                    rateLimited = true
                    this.bot.logger.debug(this.bot.isMobile, 'SEARCH-CHINA', `Source ${source} was rate limited`)
                    continue
                }

                const topics = parseChinaHotWords(response.data)
                if (topics.length) return topics
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CHINA',
                    `Source ${source} returned no usable topics | body=${summarizeChinaBody(response.data)}`
                )
            } catch (error) {
                const typed = error as { status?: number; response?: { status?: number; data?: unknown } }
                const status = typed.status ?? typed.response?.status
                const body = typed.response?.data
                if (isChinaRateLimited(status, body)) rateLimited = true
                this.bot.logger.debug(
                    this.bot.isMobile,
                    'SEARCH-CHINA',
                    `Source ${source} failed${status ? ` (${status})` : ''} | ${summarizeChinaBody(body ?? error)}`
                )
            }
        }
