...
import { loadJsonAccounts, migrateLegacyAccounts, migrateLegacyConfig } from './LegacyConfigMigration'
...
export function loadAccounts(): Account[] {
    try {
        ...
        const indexes = getAccountIndexes()
        if (!indexes.length) {
            const projectRoot = getProjectRoot()
            migrateLegacyAccounts(projectRoot)
            const accountsPath = resolveProjectFile('accounts.json')
            if (!accountsPath) {
                throw new Error(
                    'No accounts found. Set ACCOUNT_N_EMAIL / ACCOUNT_N_PASSWORD (see env.example) or provide accounts.json.'
                )
            }
            console.warn('[Accounts] Loading accounts.json compatibility mode; migrate to .env when practical.')
            return validateAccounts(loadJsonAccounts(accountsPath))
        }
...
export function loadConfig(): Config {
    try {
        ...
        migrateLegacyConfig(getProjectRoot())
