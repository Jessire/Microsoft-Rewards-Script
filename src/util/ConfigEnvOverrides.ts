...
export const ENV_OVERRIDES: EnvOverrideEntry[] = [
    ...
    { env: 'CONFIG_SEARCH_CHINA_APPKEY', path: 'searchSettings.chinaApi.appkey', type: 'string' },
...
export function formatOverrideValueForLog(env: string, value: unknown): string {
    return /(?:TOKEN|APPKEY|PASSWORD|SECRET)/i.test(env) ? '***REDACTED***' : JSON.stringify(value)
}
    ...
    if (command === 'apply') {
        ...
        try {
            ...
            report.applied.forEach(a =>
                console.log(`[entrypoint]   .${a.path} = ${formatOverrideValueForLog(a.env, a.value)}`)
            )
