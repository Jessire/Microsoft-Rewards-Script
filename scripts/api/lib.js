...
export function redactSecrets(config) {
    ...
    if (wh && typeof wh === 'object') {
        ...
        if (wh.pushplus?.token) wh.pushplus.token = mask
    }
    const chinaApi = clone.searchSettings?.chinaApi
    if (chinaApi && typeof chinaApi === 'object' && chinaApi.appkey) {
        chinaApi.appkey = mask
    }
    return clone
}
