...
Webhook URLs, tokens, chat identifiers, `webhook.pushplus.token`, and
`searchSettings.chinaApi.appkey` are replaced with `***REDACTED***` by default.
...
Compatibility settings can be patched with the same recursive merge behavior:
...
curl --request PATCH \
  ...
  --header 'Authorization: Bearer YOUR_API_TOKEN' \
  --header 'Content-Type: application/json' \
  --data '{"searchSettings":{"queryEngines":["china","local"],"chinaApi":{"appkey":"YOUR_APPKEY"}},"webhook":{"pushplus":{"enabled":true,"token":"YOUR_PUSHPLUS_TOKEN"}}}'
```

The China API key and PushPlus token are masked by subsequent `GET /config` responses unless the explicit reveal controls described above are enabled.
