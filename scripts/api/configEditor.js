...
function structuralValidate(cfg) {
    ...
    if ('searchSettings' in cfg) {
        if (!isObject(cfg.searchSettings)) {
                ...
                (!Array.isArray(search.queryEngines) ||
                    search.queryEngines.some(
                        engine =>
                            ...
                            !/^(?:google|wikipedia|wikirandom|hackernews|reddit|local|china|rss(?:\.[A-Za-z0-9_-]+){0,2})$/.test(
                                engine
                            )
                    ))
            ) {
            ...
            if ('chinaApi' in search) {
                if (!isObject(search.chinaApi)) {
                    errors.push('searchSettings.chinaApi must be an object')
                } else if ('appkey' in search.chinaApi && typeof search.chinaApi.appkey !== 'string') {
                    errors.push('searchSettings.chinaApi.appkey must be a string')
                }
            }
    ...
    if ('webhook' in cfg) {
        if (!isObject(cfg.webhook)) {
            ...
            for (const key of ['discord', 'ntfy', 'telegram', 'pushplus']) {
